...
session_start();
$sql = "SELECT bai_viet . ma_bv, bai_viet . ten_bv, MIN(binh_luan_bv . ngay_bl) as cu_nhat, MAX(binh_luan_bv . ngay_bl) as moi_nhat, COUNT(binh_luan_bv . noi_dung) as so_luong_cmt FROM binh_luan_bv INNER JOIN bai_viet ON binh_luan_bv . ma_bv = bai_viet . ma_bv GROUP BY bai_viet . ma_bv, bai_viet . ten_bv HAVING so_luong_cmt > 0";
$data = pdo_query($sql);
$sql1 = "SELECT COUNT(noi_dung) FROM binh_luan_bv";
...
<?php require_once "./../side_bar/side_bar.php"; ?>
  ...
  <div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
    ...
    <div class="row">
      <div class="col-lg-12">
        <h1 class="page-header">Bình luận bài viết</h1>
      </div>
    </div>
    <!--/.row-->
            ...
            <form action="\du_an_1\admin-template-master\8.cmt_bv\handle.php" method="post">
              ...
              <?php foreach ($data as $cmt) { ?>
                <tr class="text-left">
                  ...
                  <td width="2%">
                    <a href="\du_an_1\admin-template-master\8.cmt_bv\detail_comment.php?ma_bv=<?=$cmt['ma_bv']?>" class="btn btn-sm w-100 btn-warning text-white">Chi tiết</a>
                  </td>
                  <td width="2%">
                    <a href="#" data-href="\du_an_1\admin-template-master\8.cmt_bv\handle.php?ma_bv=<?=$cmt['ma_bv']?>" data-target="#confirm-delete" data-toggle="modal"
